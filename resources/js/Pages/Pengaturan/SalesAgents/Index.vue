...
// Tampilan dikelompokkan per asal (kategori); urutan geser berlaku di dalam
// kategorinya saja. Kategori kosong tidak ditampilkan.
const groups = computed(() =>
    Object.entries(props.tipeOptions)
        .map(([tipe, label]) => ({ tipe, label, items: props.agents.filter(a => a.tipe === tipe) }))
        .filter(g => g.items.length)
);

const tipeBadgeCls = (tipe) => tipe === 'inhouse' ? 'bg-blue-500/15 text-blue-400' : 'bg-indigo-500/15 text-indigo-400';
        ...
        <div class="space-y-5">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-400 text-sm mt-0.5">Nama dan asal sales/agen — untuk tagging atribusi/rekap, bukan pengelolaan kontak atau komisi</p>
                </div>
            ...
            <div v-if="!groups.length" class="bg-slate-900 border border-slate-800 rounded-xl px-5 py-12 text-center text-slate-500 text-sm">
                Belum ada data sales/agent.
            </div>

            <section v-for="group in groups" :key="group.tipe" class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                <header class="flex items-center gap-2 px-5 py-3 border-b border-slate-800 bg-slate-800/30">
                    <span :class="tipeBadgeCls(group.tipe)" class="px-2 py-0.5 rounded-full text-xs font-medium">{{ group.label }}</span>
                    <span class="text-slate-500 text-xs">{{ group.items.length }} orang</span>
                </header>

                <div class="divide-y divide-slate-800/70">
                    <div v-for="(agent, idx) in group.items" :key="agent.id"
                        ...
                        <div class="flex items-center gap-3">
                            <UrutanButtons type="sales-agent" :id="agent.id" :first="idx === 0" :last="idx === group.items.length - 1" />
                            <span class="text-slate-200 text-sm font-medium">{{ agent.nama }}</span>
                        </div>
