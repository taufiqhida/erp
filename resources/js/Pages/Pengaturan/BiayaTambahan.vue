...
const addPreset = () => {
    addForm.post(route('pengaturan.biaya-tambahan.store'), {
        onSuccess: () => addForm.reset(),
    });
};

const toggleActive = (preset) => {
    router.patch(route('pengaturan.biaya-tambahan.update', preset.id), {
        nama: preset.nama,
...
const destroy = (preset) => {
    if (confirm(`Hapus preset "${preset.nama}"?`)) {
        delForm.delete(route('pengaturan.biaya-tambahan.destroy', preset.id));
    }
};
</script>

<template>
    <PengaturanLayout title="Preset Biaya Tambahan">
            <div>
                <h1 class="text-white font-bold text-xl">Preset Biaya Tambahan</h1>
                <p class="text-slate-400 text-sm mt-0.5">Daftar nama biaya tambahan (kavling hook, strategis, dll) yang bisa dipilih sales saat booking — nominalnya diisi fleksibel per unit saat itu, bukan di sini</p>
            </div>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                ...
                <div v-else class="px-5 py-8 text-center text-slate-600 text-sm">
                    Belum ada preset biaya tambahan.
                </div>

                <!-- Add New -->
                <div class="px-5 py-4 border-t border-slate-800 bg-slate-800/20">
                    <div class="flex gap-3">
                        <input v-model="addForm.nama" type="text" placeholder="Nama biaya (mis. Kavling Hook)"
